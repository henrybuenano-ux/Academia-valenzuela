...
- ✅ **2 workflows pegamento** también creados en borrador: "Lead de landing → Captación" (tag `lead-landing-133` → oportunidad en Nuevo lead) y "Lead del bot → Captación" (tag `lead-bot` → oportunidad en Cualificado); paso create_opportunity clonado del espejo (create-or-update, no duplica). Total: 8 workflows en la sub-cuenta.
- ✅ Infraestructura GHL completada por API: +6 tags (lead-frio-133, no-133, dunning-avisos-completados, asesoria-agendada, matriculado-133, lead-bot) y +4 custom fields (UTM Source/Medium/Campaign + "Situación oposición"). Pipelines ya estaban completos. **Falta solo: calendario "Asesorías" (0 en la sub-cuenta — decidir quién/horario) y DNS del dominio de email → D1 lunes.**
- Remates de UI pendientes (~20 min, anotados en ClickUp): goal events de salida, notificación interna en los 2 pegamento, enlace real del calendario, ventana de envío 10-20 h, email B2 con novedades de Paco, form→tag lead-landing-133, bot→tag lead-bot.
